...
const header=$(".header");const progress=$(".progress");
function scroll(){header.classList.toggle("scrolled",scrollY>35);const max=document.documentElement.scrollHeight-innerHeight;progress.style.width=max>0?(scrollY/max*100)+"%":"0%"}addEventListener("scroll",scroll,{passive:true});scroll();
...
if(!reduce&&!matchMedia("(pointer:coarse)").matches){
 ...
 function cursor(){rx+=(tx-rx)*.18;ry+=(ty-ry)*.18;gx+=(tx-gx)*.08;gy+=(ty-gy)*.08;ring.style.left=rx+"px";ring.style.top=ry+"px";dot.style.left=tx+"px";dot.style.top=ty+"px";glow.style.left=gx+"px";glow.style.top=gy+"px";requestAnimationFrame(cursor)}cursor();
 $$("a,.magnetic-card").forEach(el=>{el.addEventListener("mouseenter",()=>ring.classList.add("active"));el.addEventListener("mouseleave",()=>ring.classList.remove("active"))});
 ...
 $$(".magnetic-card").forEach(card=>card.addEventListener("mousemove",e=>{const r=card.getBoundingClientRect();const x=e.clientX-r.left-r.width/2,y=e.clientY-r.top-r.height/2;card.style.transform="perspective(1000px) rotateX("+(-y/r.height*5)+"deg) rotateY("+(x/r.width*5)+"deg) translateY(-6px)";card.style.setProperty("--mx",e.clientX-r.left+"px");card.style.setProperty("--my",e.clientY-r.top+"px")}));
 $$(".magnetic-card").forEach(card=>card.addEventListener("mouseleave",()=>card.style.transform=""));
}
...
const ticker=$(".ticker");if(ticker&&!reduce){const items=ticker.innerHTML;ticker.innerHTML=items+items;let x=0;function tick(){x-=.35;if(Math.abs(x)>ticker.scrollWidth/2)x=0;ticker.style.transform="translateX("+x+"px)";requestAnimationFrame(tick)}tick()}
});